...
instance Ranged CellRef where
    data Dimension CellRef
       = Dimension { columns :: [Char]
                   , rows :: [Integer] }
    cellRange dim ran@(Box (Cell ulc ulr) (Cell lrc lrr))
        = Set.fromList [Cell c r | c <- [ulc .. lrc], r <- [ulr .. lrr], elem c (columns dim), elem r (rows dim)]
  ...
  { name = "Sheet2",
    ...
    content = 
      Map.fromList
        [ ((Cell 'A' 1),  Constant 12),
          ((Cell 'A' 2),  Constant 2),
          ((Cell 'B' 1),  Mul (Constant 4) (Ref (Cell 'A' 1))),
          ((Cell 'B' 2),  Constant 4),
...
evaluate sheet (Ref cell) = case (Map.lookup cell (content sheet)) of
  Just x -> evaluate sheet x
  Nothing -> Nothing

evaluate sheet (Sum cells) = 
  let sumMaybe = fmap sum . sequence
      cellList = Set.toList $ cellRange (dimension sheet) cells
  in sumMaybe $ map (evaluate sheet) (map Ref cellList)

evaluate sheet (Add expr1 expr2) = case (evaluate sheet expr1, evaluate sheet expr2) of
  (Just x, Just y) -> Just (x + y)
  rest -> Nothing

evaluate sheet (Mul expr1 expr2) = case (evaluate sheet expr1, evaluate sheet expr2) of
  (Just x, Just y) -> Just (x * y)
  rest -> Nothing
...
pAdd = pOperator "+" Add (pTerm <|> pMul)


exactString :: String -> Parser ()
exactString [] = return ()
exactString (c:cs) = do 
  exactChar c 
  exactString cs
