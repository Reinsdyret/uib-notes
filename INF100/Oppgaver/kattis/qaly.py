...
The quality of life lived can be quantified as a number between 0 and 1. If someone is living with perfect health, the quality of life is 1. 
If someone is dead, then the quality of life is 0. The quality of life may increase or decrease due to medical treatements, sickness, etc.
...
for i in range(0,N):
    qualityQuantityList.append(input().split())

for i in range(0,len(qualityQuantityList)):
    qalyList += float(qualityQuantityList[i][0]) * float(qualityQuantityList[i][1])

print("{:.3f}".format(qalyList))
