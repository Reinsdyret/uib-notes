...
The next N lines describe the times the stopwatch’s button was pressed in increasing order. Each line contains a single integer t (0≤t≤106), which is the time the button was pressed. 
No two button presses happen on the same second.
...
sumMinutes = 0
for i in range(0,N):
    pressed.append(int(input()))

if N % 2 != 0:
    print("still running")
else:
    for i in range(0,len(pressed),2):
        sumMinutes += pressed[i+1] - pressed[i]
