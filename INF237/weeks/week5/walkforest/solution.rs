...
fn number_of_valid_paths(adj_list: &Vec<Vec<Edge>>, start: usize, goal: usize) -> i32 {
    ...
    let mut paths: Vec<i32> = (0..adj_list.len()).map(|_| 0).collect();
    ...
    while let Some(state @ State { cost, position }) = heap.pop() {
        ...
        for edge in &adj_list[position] {
            ...
            } else if dist[next.position] < dist[position] {
                //println!("{} {}", paths[position], paths[next.position]);
                paths[position] += paths[next.position];
            }
        }
    }
...
fn main() {
    ...
    while line.trim() != "0" {
        let [n, m]: [usize; 2] = line
            .trim()
            .split(' ')
            .map(|c| c.parse().unwrap())
            .collect::<Vec<usize>>()
            .try_into()
            .expect("Expected n and m");
        ...
        println!("{}", number_of_valid_paths(&adj_list, 2, 1));
