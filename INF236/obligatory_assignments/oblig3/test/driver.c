...
void store_p_value(),store_p_value_latex();
// Removed BFS verification; not needed for MIS algorithms
// int verify_bfs();
int verify_mis(int n, int *ver, int *edges, int *mis);
...
// OpenMP locks, not needed in INF236
  ...
  // for (lock_i = 0; lock_i < max_n; lock_i++) {
  //   omp_init_lock(&nlocks[lock_i]);
  // }
...
//
  ...
  int i=0,j=0;      // Initialize loop indices
  double mt1,mt2;   // Used for timing of each algorithm
  
// Boolean variables for specifying which algorithms to run 
// Currently there are three available algorithms for maximal independent set.
// Only the algorithms where the boolean value is set to true are run
//
  int SMIS              = true;    /* Sequential MIS */
  int ALUBYS            = false;    /* Alternative Lubys */
  int LUBYS             = true;    /* Luby's parallel MIS algorithm */
...
// Allocating memory for sequential graph algorithms
  if (!allocate_graph_memory(&weight,&e,&ver,&edges,max_n,&p,&p1,&p2,&p3,&p4,&p5,&p6,&p7)) {			
    return(false);
  }
...
// Upon return p indicates membership in the MIS

    if (SMIS) {
      for(l=0;l<n_runs;l++) {
        mt1 = omp_get_wtime();      // Get current time
        smis(n[i],ver,edges,p,p1,p2,p3);
      ...
      printf("************************************************\n");
      printf("*  Sequential MIS                              *\n");
      printf("*  Time = %8.6lf seconds                     *\n",timer[0][i]);
      printf("************************************************\n");
     // Verify maximal independent set for Sequential MIS
     if (!verify_mis(n[i],ver,edges,p)) {
         printf("Error verifying Sequential MIS on graph %s\n", name[i]);
         return(false);
     }
    }
...
#pragma omp barrier
        for(k=0;k<n_runs;k++) {   // Run all experiments n_runs times, select best run time as final
          ...
          if (ALUBYS) {   // Parallel MIS
#pragma omp barrier
            { mt1 = omp_get_wtime(); }
            lubys_balanced_parallel(n[i],ver,edges,p,p2,p3,p4,p5,p6,p7);
...
#pragma omp master
        ...
        // Verify maximal independent set for Parallel MIS
        #pragma omp master
        if (ALUBYS)
          verify_mis(n[i],ver,edges,p);
...
#pragma omp barrier
        for(k=0;k<n_runs;k++) {   // Run all experiments n_runs times, select best run time as final
          ...
          if (LUBYS) {   // Luby's parallel MIS algorithm
#pragma omp barrier
#pragma omp master
            { mt1 = omp_get_wtime(); }
            lubys(n[i], ver, edges, p, p2, p3, p4);
...
#pragma omp master
        ...
        // Verify maximal independent set for Luby's algorithm
        #pragma omp master
        if (LUBYS)
          verify_mis(n[i],ver,edges,p);
...
//
  ...
  if (SMIS)
    store_value(wf,"TimeSequentialMIS",timer[0],n_graphs);    // Print timing results for sequential MIS

// Print out parallel values

  if (ALUBYS)
    store_p_value(wf,"TimeParallelMIS",p_timer[0],n_graphs,n_conf); // Print timing results for parallel MIS
    store_p_value_latex(wf, "Parallel MIS", p_timer[0], timer[0], n_graphs, n_conf, conf, name, n, m);

  if (LUBYS)
    store_p_value(wf,"TimeParallelLUBYS",p_timer[1],n_graphs,n_conf); // Print timing results for Luby's algorithm
    store_p_value_latex(wf, "Parallel LUBYS", p_timer[1], timer[0], n_graphs, n_conf, conf, name, n, m);
...
// Verify maximal independent set: checks independence and maximality
int verify_mis(int n, int *ver, int *edges, int *mis) {
    int v;
    // Check independence: no edge with both endpoints in MIS
    for (v = 1; v <= n; v++) {
        if (mis[v]) {
            int start = ver[v];
            int end = ver[v+1];
            for (int idx = start; idx < end; idx++) {
                int u = edges[idx];
                if (mis[u]) {
                    printf("Error: vertices %d and %d are both in MIS\n", v, u);
                    return false;
                }
            }
        }
    }
    // Check maximality: every non-selected vertex has a neighbor in MIS
    for (v = 1; v <= n; v++) {
        if (!mis[v]) {
            int start = ver[v];
            int end = ver[v+1];
            ...
            if (!found) {
                printf("Error: vertex %d can be added to MIS (not maximal)\n", v);
                return false;
            }
        }
    }
...
void store_p_value_latex(FILE *wf, char *algorithm_name, double **values, double *seq_values, 
  int n_graphs, int n_conf, int *conf, char **graph_names, int *n, int *m) {
// Create one table per graph
for (int i = 0; i < n_graphs; i++) {
// Extract just the filename from the path
char *graph_name = strdup(graph_names[i]);
char *graph_short_name = graph_name;

// Find the last '/' character to get just the filename
char *last_slash = strrchr(graph_name, '/');
if (last_slash != NULL) {
graph_short_name = last_slash + 1;
}

// Remove file extension if present
char *dot = strchr(graph_short_name, '.');
if (dot != NULL) {
*dot = '\0';
}

// Create a LaTeX-safe version of the graph name (escape underscores)
char *latex_name = (char *)malloc(strlen(graph_short_name) * 2 + 1);
char *src = graph_short_name;
char *dst = latex_name;
...
fprintf(wf, "\\begin{table}[ht]\n");
fprintf(wf, "\\centering\n");
fprintf(wf, "\\caption{%s - %s ($|V| = %d$, $|E| = %d$)}\n", 
algorithm_name, latex_name, n[i], m[i]);
fprintf(wf, "\\begin{tabular}{l|ccc}\n\\toprule\n");

// Header row
fprintf(wf, "Threads & Time (s) & Speedup vs Sequential & Relative Speedup \\\\\n");
fprintf(wf, "\\midrule\n");

// Sequential timing row (if available)
if (seq_values != NULL) {
fprintf(wf, "Sequential & %.6f & 1.00 & --- \\\\\n", seq_values[i]);
}

// Data rows for each thread configuration
for (int j = 0; j < n_conf; j++) {
double time = values[i][j];
double seq_speedup = (seq_values != NULL) ? seq_values[i] / time : 0.0;
double rel_speedup = values[i][0] / time;

// Color cells based on speedup
fprintf(wf, "%d ", conf[j]);

if (seq_speedup > 1.8) {
fprintf(wf, "& \\cellcolor{green!25}%.6f & \\cellcolor{green!25}%.2f & \\cellcolor{green!25}%.2f \\\\\n", 
 time, seq_speedup, rel_speedup);
} else if (seq_speedup > 1.0) {
fprintf(wf, "& \\cellcolor{yellow!25}%.6f & \\cellcolor{yellow!25}%.2f & \\cellcolor{yellow!25}%.2f \\\\\n", 
 time, seq_speedup, rel_speedup);
} else {
fprintf(wf, "& \\cellcolor{red!25}%.6f & \\cellcolor{red!25}%.2f & \\cellcolor{red!25}%.2f \\\\\n", 
 time, seq_speedup, rel_speedup);
}
}

fprintf(wf, "\\bottomrule\n");
fprintf(wf, "\\end{tabular}\n");
fprintf(wf, "\\end{table}\n\n");
...
int allocate_graph_memory(double **weight,edge **e,int **ver,int **edges,int n,int **p,int **p1,int **p2,int **p3, int **p4, int **p5, int **p6, int **p7) {		
...
// The raw edge list is stored in e, i.e. in the same order as it was read in
  ...
  *p5 = (int *) malloc(sizeof(int)*(n+2));	// Extra storage of length n+2
  if (*p5 == NULL) {
    printf("Unable to allocate space for p5[] in allocate_graph_memory() \n");
    return(false);
  }

  *p6 = (int *) malloc(sizeof(int)*(n+2));	// Extra storage of length n+2
  if (*p6 == NULL) {
    printf("Unable to allocate space for p6[] in allocate_graph_memory() \n");
    return(false);
  }

  *p7 = (int *) malloc(sizeof(int)*(n+2));	// Extra storage of length n+2
  if (*p7 == NULL) {
    printf("Unable to allocate space for p7[] in allocate_graph_memory() \n");
    return(false);
  }
