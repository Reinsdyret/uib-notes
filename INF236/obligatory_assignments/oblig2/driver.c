...
// Print out parallel values
  ...
  if (PBFS) 
    ...
    store_p_value_latex(wf, "Parallel BFS", p_timer[0], timer[0], n_graphs, n_conf, conf, name, n, m);

  if (ABFS) 
    store_p_value(wf,"TimeAParallelBFS",p_timer[1],n_graphs,n_conf); // Print timing results for alt parallel BFS
    store_p_value_latex(wf, "Alternative Parallel BFS", p_timer[1], timer[0], n_graphs, n_conf, conf, name, n, m);
